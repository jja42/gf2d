...
break;
case 3: load_wormmon(self,position,gfc_vector2d(flip,0));
break;
...
void load_betamon(Enemy* self,GFC_Vector2D position, int patrol_bound_left, int patrol_bound_right){
	...
	self->ent->data = self;
	self->ent->gravity = 1;
	gf2d_entity_load(self->ent,"images/betamon.png",25,24,5,position,gfc_vector2d(3,3));
	self->ent->frame = 0;
	...
	self->ent->box = gf2d_box(self->ent->position, 33, 30, gfc_vector2d(42,39));
...
void load_penguinmon(Enemy* self,GFC_Vector2D position, int patrol_bound_left, int patrol_bound_right){
	...
	self->ent->data = self;
	self->ent->gravity = 1;
	gf2d_entity_load(self->ent,"images/penguinmon.png",24,24,6,position,gfc_vector2d(3,3));
	self->ent->frame = 0;
	...
	self->patrol_bound_left = patrol_bound_left;
	self->patrol_bound_right = patrol_bound_right;
	self->ent->box = gf2d_box(self->ent->position, 30, 30, gfc_vector2d(36,36));
...
void load_wormmon(Enemy* self,GFC_Vector2D position, GFC_Vector2D flip){
	...
	self->ent->data = self;
	self->ent->gravity = 1;
	gf2d_entity_load(self->ent,"images/wormmon.png",16,24,5,position,gfc_vector2d(3,3));
	self->ent->frame = 0;
	...
	self->ent->flip = flip;
	self->ent->timer = 0;
	self->ent->box = gf2d_box(self->ent->position, 18, 30, gfc_vector2d(24,39));
...
void load_zubat(Enemy* self,GFC_Vector2D position, int patrol_bound_left, int patrol_bound_right){
	...
	self->patrol_bound_left = patrol_bound_left;
	self->patrol_bound_right = patrol_bound_right;
	gf2d_entity_load(self->ent,"images/zubat.png",32,34,3,self->ent->position,gfc_vector2d(3,3));
	self->ent->frame = 0;
	self->move_end_frame = 2;
	self->attack_start_frame = 0;
	self->attack_end_frame = 0;
	self->ent->box = gf2d_box(self->ent->position, 30, 48, gfc_vector2d(48,51));
	self->enemy_type = 4;
...
void load_pikachu(Enemy* self,GFC_Vector2D position, int patrol_bound_left, int patrol_bound_right){
	...
	self->patrol_bound_right = patrol_bound_right;
	self->ent->gravity = 1;
	gf2d_entity_load(self->ent,"images/pikachu.png",37,50,4,self->ent->position,gfc_vector2d(3,3));
	self->ent->frame = 0;
	self->move_end_frame = 4;
	self->attack_start_frame = 5;
	self->attack_end_frame = 7;
	self->ent->box = gf2d_box(self->ent->position, 33, 54, gfc_vector2d(57,96));
	self->enemy_type = 5;
...
void load_articuno(Enemy* self,GFC_Vector2D position, int patrol_bound_left, int patrol_bound_right){
	...
	self->ent->data = self;
	self->ent->gravity = 1;
	gf2d_entity_load(self->ent,"images/articuno.png",71,55,3,self->ent->position,gfc_vector2d(3,3));
	self->ent->frame = 0;
	self->move_end_frame = 2;
	self->attack_start_frame = 2;
	self->attack_end_frame = 2;
	self->ent->box = gf2d_box(self->ent->position, 48, 63, gfc_vector2d(108,81));
	self->enemy_type = 6;
...
void penguinmon_think(Entity* self){
	...
	if(self->timer == 0){
	if(self->flip.x == 0)gf2d_projectile_spawn("images/enemy_attack.png",8,6,3,gfc_vector2d(self->position.x+50,self->position.y+20),gfc_vector2d(3,3),gfc_vector2d(2.5,0),self->flip,gfc_vector2d(12,6),12,6,2,30);
	if(self->flip.x == 1)gf2d_projectile_spawn("images/enemy_attack.png",8,6,3,gfc_vector2d(self->position.x,self->position.y+20),gfc_vector2d(3,3),gfc_vector2d(-2.5,0),self->flip,gfc_vector2d(12,6),12,6,2,30);
	self->timer = 200;
	self->frame = 0;
...
void wormmon_think(Entity* self){
	self->gravity = 0;
	if(self->timer == 0){
	if(self->flip.x == 0)gf2d_projectile_spawn("images/enemy_attack.png",8,6,3,gfc_vector2d(self->position.x+25,self->position.y+30),gfc_vector2d(3,3),gfc_vector2d(2.5,0),self->flip,gfc_vector2d(12,6),12,6,2,30);
	if(self->flip.x == 1)gf2d_projectile_spawn("images/enemy_attack.png",8,6,3,gfc_vector2d(self->position.x,self->position.y+30),gfc_vector2d(3,3),gfc_vector2d(-2.5,0),self->flip,gfc_vector2d(12,6),12,6,2,30);
	self->timer = 250;
	gfc_sound_play(EnemyShoot,0,.25,3,1);}
	self->timer--;
}
...
void articuno_think(Entity *self){
	...
	if(self->timer == 0){
	int rando =	rand() % 2;
	if(rando == 0)gf2d_projectile_spawn("images/articuno_attack.png",10,20,3,gfc_vector2d(self->position.x+130,self->position.y+150),gfc_vector2d(2,2),gfc_vector2d(0,2),self->flip,gfc_vector2d(10,24),8,16,5,30);
	if(rando == 1)gf2d_projectile_spawn("images/articuno_attack.png",10,20,3,gfc_vector2d(self->position.x+50,self->position.y+150),gfc_vector2d(2,2),gfc_vector2d(0,2),self->flip,gfc_vector2d(10,24),8,16,5,30);
	self->timer = 150;
...
void pikachu_think(Entity *self){
	...
	if(e->timer == 1){
		...
		if(self->frame > e->attack_end_frame) {
			self->frame = 0;
			gf2d_projectile_spawn("images/pikachu_attack.png",22,203,3,gfc_vector2d(self->position.x,self->position.y-500),gfc_vector2d(2,2),gfc_vector2d(0,2),self->flip,gfc_vector2d(24,204),16,202,4,30);
			e->timer = 0;
